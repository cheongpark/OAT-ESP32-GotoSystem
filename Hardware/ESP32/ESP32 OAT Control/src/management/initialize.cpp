...
bool Initialize::connectMount() {
    ...
    try {
        ...
        response = mount_serial.request("XGR");
        if (response.success) {
            Mount::Data::set_steps_ra(response.message);
        }

        response = mount_serial.request("XGD");
        if (response.success) {
            Mount::Data::set_steps_dec(response.message);
        }

        response = mount_serial.request("XGS");
        if (response.success) {
            Mount::Data::set_speed_factor(response.message);
        }

        response = mount_serial.request("XGL");
        if (response.success) {
            Mount::Data::set_lst(response.message);
        }

        response = mount_serial.request("Gg");
        if (response.success) {
            Mount::Data::set_longitude(response.message);
        }

        response = mount_serial.request("Gt"); 
        if (response.success) {
            Mount::Data::set_latitude(response.message);
        }
        ...
        // 연결 성공
        Mount::Data::set_connected(true);

        return true;
        
    } catch (std::exception& e) {
        // 연결 실패
        LOG(LOG_ERROR, "마운트 연결 실패: " + String(e.what()));
        Mount::Data::set_connected(false);
